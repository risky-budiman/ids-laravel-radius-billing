    ...
    >
        ...
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-lg border border-gray-100/50 dark:border-gray-700/50 p-8 overflow-hidden relative">
            ...
            <div class="relative z-10">
                ...
                <form action="{{ route('locations.stb.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-6 gap-6 items-end">
                    @csrf
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="text" name="name" placeholder="E.g. Distribution Box" class="block w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-gray-700 dark:text-gray-300 shadow-sm focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 transition-all px-4 py-3" required />
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <button type="submit" class="w-full bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-6 py-3 rounded-xl hover:from-cyan-700 hover:to-blue-700 font-bold shadow-lg shadow-cyan-500/30 transform hover:-translate-y-0.5 transition-all">
                            Save STB
                        </button>
                    </div>
                </form>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                        ...
                        <tr class="hover:bg-cyan-50/30 dark:hover:bg-cyan-900/10 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-xs font-mono text-gray-500">
                                @if($t->latitude)
                                    <span class="block">Lat: {{ $t->latitude }}</span>
                                    <span class="block">Lon: {{ $t->longitude }}</span>
                                @else
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end items-center gap-3">
                                    <button 
                                        @click='editStb = @json($t); editModalOpen = true'
                                        class="text-cyan-600 hover:text-cyan-900 font-bold text-sm"
                                    >
                                        Edit
                                    </button>
                                    <form action="{{ route('locations.stb.destroy', $t) }}" method="POST">
                                        @csrf @method('DELETE')
                                        <button class="text-red-500 hover:text-red-700 font-bold text-sm" onclick="return confirm('Delete STB?');">
                                            Delete
                                        </button>
                                    </form>
                                </div>
        ...
        <div x-show="editModalOpen" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-lg p-8 relative z-10 border border-gray-100 dark:border-gray-700">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edit STB: <span x-text="editStb.code" class="text-cyan-500"></span></h3>
                    
                    <form :action="`{{ url('admin/locations/stbs') }}/${editStb.id}`" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Parent STO</label>
                            <select name="sto_id" x-model="editStb.sto_id" class="block w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-gray-700 dark:text-gray-300 px-4 py-3" required>
                                @foreach($stos as $s)
                                    <option value="{{ $s->id }}">[{{ $s->code }}] {{ $s->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">STB Name</label>
                            <input type="text" name="name" x-model="editStb.name" class="block w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-gray-700 dark:text-gray-300 px-4 py-3" required />
                        </div>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <div class="relative">
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Latitude</label>
                                <input type="text" name="latitude" id="edit_lat" x-model="editStb.latitude" class="block w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-gray-700 dark:text-gray-300 px-4 py-3" />
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Longitude</label>
                                <input type="text" name="longitude" id="edit_lon" x-model="editStb.longitude" class="block w-full rounded-xl border-gray-200 dark:border-gray-700 dark:bg-gray-900 text-gray-700 dark:text-gray-300 px-4 py-3" />
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end gap-3 mt-8">
                            ...
                            <button type="submit" class="px-10 py-3 bg-cyan-600 text-white rounded-xl font-bold shadow-lg">Update STB</button>
                        </div>
                    </form>
                </div>
            </div>

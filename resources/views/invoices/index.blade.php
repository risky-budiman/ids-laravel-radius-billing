<x-app-layout>
    ...
    <div class="mb-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <form action="{{ route('invoices.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div class="md:col-span-2">
                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Pencarian</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari No. Invoice atau Nama Pelanggan..." 
            ...
            <div class="flex space-x-2">
                <button type="submit" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-sm font-bold transition-all shadow-md shadow-indigo-600/20">
                    Filter
                </button>
                                ...
                                <div class="flex items-center justify-end space-x-3">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            @if($activeGateways->count() > 1)
                                                ...
                                                <div class="relative group">
                                                    <button class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all shadow-md shadow-indigo-600/30 flex items-center">
                                                        Pay Online
                                                        <svg class="w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                                    </button>
                                                    <div class="absolute z-20 hidden group-hover:block right-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-600 p-2 transform origin-top-right transition-all">
                                                        @foreach($activeGateways as $gw)
                                                            <a href="{{ route('invoices.pay', ['invoice' => $invoice, 'gateway' => $gw->provider]) }}" target="_blank" class="block px-4 py-2 text-[10px] font-black uppercase tracking-wider text-gray-800 dark:text-gray-200 hover:bg-indigo-600 hover:text-white dark:hover:bg-indigo-600 rounded-lg transition-all mb-1 last:mb-0">
                                            ...
                                            @elseif($activeGateways->count() == 1)
                                                ...
                                                <a href="{{ route('invoices.pay', ['invoice' => $invoice, 'gateway' => $activeGateways->first()->provider]) }}" target="_blank" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all shadow-md shadow-indigo-600/30 transform hover:-translate-y-0.5 active:translate-y-0">
                                                    Pay Online
                                                </a>
                                            @endif
                                            ...
                                            <form action="{{ route('invoices.update', $invoice) }}" method="POST">
                                                ...
                                                <button type="submit" class="bg-emerald-50 hover:bg-emerald-100 text-emerald-600 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors border border-emerald-100">Mark Paid</button>
                                            </form>
                                    ...
                                    @elseif($invoice->status == 'paid')
                                        <form action="{{ route('invoices.update', $invoice) }}" method="POST">
                                            ...
                                            <button type="submit" class="text-amber-600 hover:text-amber-900 dark:text-amber-400 dark:hover:text-amber-300 text-xs font-bold uppercase tracking-wider" onclick="return confirm('Cancel this payment and revert to unpaid?');">Cancel Payment</button>
                                        </form>
                                    @endif

    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-6">
            ...
            <div class="glass bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-4">
                <h4 class="text-xs font-black text-gray-400 uppercase tracking-widest">Portal Account</h4>
                @if($customer->user_id)
                    <div class="flex items-center space-x-3 p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl border border-emerald-100 dark:border-emerald-800">
                        <div class="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-emerald-700">Account Active</p>
                            <p class="text-[10px] text-emerald-600/70">{{ $customer->user->email }}</p>
                        </div>
                    </div>
                @else
                    <div class="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 text-center">
                        <p class="text-[10px] text-gray-400 mb-3 italic">This subscriber doesn't have a portal account yet.</p>
                        <form action="{{ route('customers.create-portal-account', $customer) }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full bg-indigo-50 hover:bg-indigo-100 text-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
                                Create Account
                            </button>
                        </form>
                    </div>

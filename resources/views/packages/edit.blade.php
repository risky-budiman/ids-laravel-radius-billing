        ...
        <form action="{{ route('packages.update', $package) }}" method="POST" class="p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2 flex items-center bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl">
                    <input type="checkbox" id="enable_fup" name="enable_fup" value="1" {{ $package->enable_fup ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-red-600 focus:ring-red-500">
                    <label for="enable_fup" class="ml-3 font-bold text-gray-700 dark:text-gray-300">Aktifkan Kebijakan FUP</label>
                </div>

                <div id="fup_details" class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-gray-50/50 dark:bg-gray-800/30 rounded-xl {{ $package->enable_fup ? '' : 'hidden' }}">
                    <div>
                        <x-input-label for="fup_limit_gb" :value="__('Quota Limit (GB)')" />
                        <x-text-input id="fup_limit_gb" name="fup_limit_gb" type="number" class="mt-1 block w-full" :value="$package->fup_limit_gb" placeholder="e.g. 500" />
                    </div>
                    <div>
                        <x-input-label for="fup_speed_limit" :value="__('Post-FUP Speed (Up/Down)')" />
                        <x-text-input id="fup_speed_limit" name="fup_speed_limit" type="text" class="mt-1 block w-full" :value="$package->fup_speed_limit" placeholder="e.g. 1M/2M" />
                    </div>
                </div>

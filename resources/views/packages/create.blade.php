    ...
    <x-slot name="header">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">Define a new internet service plan</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <div class="bg-white dark:bg-gray-800/50 backdrop-blur-xl rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-700/50">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" required placeholder="e.g. Home Basic 10M" />
                            </div>
                            <div>
                                <x-input-label for="mikrotik_group" :value="__('MikroTik Profile / Group')" />
                                <x-text-input id="mikrotik_group" name="mikrotik_group" type="text" class="mt-1 block w-full" placeholder="e.g. pppoe-10M" />
                            </div>
                            <div class="md:col-span-2">
                                <x-input-label for="description" :value="__('Public Description')" />
                                <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-200 dark:border-gray-700 dark:bg-gray-900/50 rounded-2xl focus:ring-indigo-500 focus:border-indigo-500 text-gray-700 dark:text-gray-300 transition-all" placeholder="Tell your customers about this plan..."></textarea>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-900/40 p-6 rounded-2xl border border-gray-100 dark:border-gray-800">
                                ...
                                <div class="grid grid-cols-2 gap-6">
                                    <div class="relative">
                                        ...
                                        <x-text-input id="upload_speed" name="upload_speed" type="text" class="block w-full pl-10" placeholder="Upload (e.g. 5M)" />
                                    </div>
                                    <div class="relative">
                                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
                                        ...
                                        <x-text-input id="download_speed" name="download_speed" type="text" class="block w-full pl-10" placeholder="Download (e.g. 10M)" />
                                    </div>
                                </div>
                            </div>

                            <!-- Advanced QoS (Collapsible or just clean) -->
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div class="space-y-4">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <x-text-input name="burst_limit_up" placeholder="Limit Up" class="text-sm py-2" />
                                        <x-text-input name="burst_limit_down" placeholder="Limit Down" class="text-sm py-2" />
                                        <x-text-input name="burst_threshold_up" placeholder="Thresh Up" class="text-sm py-2" />
                                        <x-text-input name="burst_threshold_down" placeholder="Thresh Down" class="text-sm py-2" />
                                        <x-text-input name="burst_time_up" placeholder="Time Up (s)" class="text-sm py-2" />
                                        <x-text-input name="burst_time_down" placeholder="Time Down (s)" class="text-sm py-2" />
                                    </div>
                                </div>
                                <div class="space-y-4">
                                    <h4 class="text-xs font-bold text-gray-500 uppercase flex items-center">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <select name="priority" id="priority" class="mt-1 block w-full border-gray-200 dark:border-gray-700 dark:bg-gray-900 rounded-xl text-sm focus:ring-indigo-500">
                                                @for($i=1; $i<=8; $i++)
                                                    <option value="{{ $i }}" {{ $i == 8 ? 'selected' : '' }}>{{ $i }} {{ $i == 1 ? '(Highest)' : ($i == 8 ? '(Lowest)' : '') }}</option>
                                                @endfor
                                            </select>
                                        </div>
                                        <div class="grid grid-cols-2 gap-4">
                                            <x-text-input name="limit_at_up" placeholder="Min Up (CIR)" class="text-sm py-2" />
                                            <x-text-input name="limit_at_down" placeholder="Min Down (CIR)" class="text-sm py-2" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="space-y-8">
                    ...
                    <div class="bg-indigo-600 rounded-3xl p-8 text-white shadow-lg shadow-indigo-500/20">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" required class="block w-full bg-white/10 border-none rounded-2xl py-4 pl-12 text-2xl font-black placeholder:text-indigo-300 focus:ring-2 focus:ring-white/30 transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" placeholder="0">
                        </div>
                    ...
                    <div class="bg-white dark:bg-gray-800/50 backdrop-blur-xl rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-700/50">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="enable_fup" id="enable_fup" value="1" class="sr-only peer">
                                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-amber-500"></div>
                            </label>
                        </div>
                        
                        <div id="fup_details" class="space-y-4 animate-fade-in hidden">
                            <div>
                                <x-input-label value="Quota Limit (GB)" />
                                <x-text-input name="fup_limit_gb" type="number" class="mt-1 block w-full text-sm" placeholder="e.g. 500" />
                            </div>
                            <div>
                                <x-input-label value="Throttled Speed" />
                                <x-text-input name="fup_speed_limit" type="text" class="mt-1 block w-full text-sm" placeholder="e.g. 1M/2M" />
                            </div>
                        </div>
                    </div>
                    @endif

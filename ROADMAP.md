...
## FASE 23: Multi-Role Staff & Hybrid Sales Capability
Fase ini bertujuan untuk memberikan fleksibilitas kepada staf (Teknisi, Admin, Kasir) agar dapat berperan ganda sebagai Sales tanpa kehilangan hak akses peran utamanya.

### 23.1. Fleksibilitas Peran (Sales-Enabled Staff)
- [ ] **Database Migration:** Penambahan kolom `is_sales_enabled` pada tabel `users`.
- [ ] **UI User Management:** Update form edit user agar Administrator dapat mengaktifkan fitur "Sales" untuk role apa pun (Teknisi/Admin/Kasir).
- [ ] **Sales Profile for All Roles:** Memberikan kolom `commission_rate` dan `bank_info` kepada semua staf yang diaktifkan fitur sales-nya.

### 23.2. Referal & Komisi Lintas Peran
- [ ] **Unified Referral Selection:** Memperbarui dropdown "Sales/Referral" pada form pendaftaran pelanggan agar menampilkan semua staf yang memiliki flag `is_sales_enabled`.
- [ ] **Universal Commission Logic:** Memastikan `SalesCommissionService` dapat menghitung bonus untuk user ID mana pun selama fitur sales-nya aktif, tanpa harus mengganti role utama user tersebut.

### 23.3. Dashboard Hibrida (Shared Dashboard Widgets)
- [ ] **Adaptive Sales Widget:** Staf (misal Teknisi) akan melihat widget "Komisi Saya" dan "Referal Saya" di dashboard utama mereka jika fitur sales diaktifkan.
- [ ] **Staff-Sales Profile:** Halaman profil staf yang menunjukkan performa teknis (misal tiket yang selesai) berdampingan dengan performa sales (pelanggan yang didaftarkan).
